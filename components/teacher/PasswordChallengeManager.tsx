...
const ChallengeForm: React.FC<{
    ...
    onSave: (data: Omit<PasswordChallenge, 'id' | 'creatorName' | 'status' | 'unlockedTimestamp' | 'classCode'>, id?: string) => Promise<boolean>;
    onCancel?: () => void;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setError('');
        if (!title.trim() || !password.trim() || !rules.trim()) {
...
export const PasswordChallengeManager: React.FC<{ selectedClass: ClassData }> = ({ selectedClass }) => {
    const { 
    ...
    const handleSaveChallenge = async (data: any, id?: string): Promise<boolean> => {
        const fullData = { ...data, classCode: selectedClass.classCode };
        const result = id ? await updatePasswordChallenge(id, fullData) : await createPasswordChallenge(fullData);
    ...
    const challengesForClass = passwordChallenges.filter(c => c.classCode === selectedClass.classCode);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
             ...
             <div className="bg-slate-900/70 p-6 rounded-lg">
                <h2 className="text-xl font-bold text-sky-300 mb-4">Desafios da Turma</h2>
                <div className="space-y-3 max-h-[26rem] overflow-y-auto pr-2">
                   {challengesForClass.length > 0 ? challengesForClass.map(c => (
                       ...
                       <p className="text-slate-400 text-center">Nenhum desafio criado para esta turma ainda.</p>
                   )}
                </div>
            </div>
        </div>
